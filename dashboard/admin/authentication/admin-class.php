...
class ADMIN
{
    ...
    {
        if($email == null) {
            echo "<script>alert('No email found'); window.location.href = '../../../index.php;</script>";
            exit();
        } else {
            ...
            if ($stmt->rowCount() > 0) {
                echo "<script>alert('Email already taken. Please try another one.); window.location.href = '../../../index.php;</script>";
                exit();
            } else {
                $_SESSION["OTP"] = $otp;
